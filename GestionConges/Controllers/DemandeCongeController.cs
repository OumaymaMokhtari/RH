...
namespace GestionConges.Controllers
{
    ...
    public class DemandeCongeController : ControllerBase
    {
        ...
        [HttpPut("modifier/{id}")]
        public async Task<IActionResult> ModifierDemande(int id, [FromBody] DemandeCongeUpdateVM request)
        {
            var demande = await _context.DemandesConges.FindAsync(id);
            if (demande == null)
                return NotFound("Demande de congé introuvable");
            ...
            demande.TypeConge = request.TypeConge;
            demande.Commentaire = request.Commentaire;

            await _context.SaveChangesAsync();
            return Ok("Demande de congé modifiée avec succès");
        }
